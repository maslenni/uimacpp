...
using namespace std;
namespace uima {
    UnicodeString const CAS::ustrCREATOR_ID_CAS(lowlevel::TypeSystem::ustrCREATOR_ID_SYSTEM);
...
namespace uima {
    ...
    int CAS::addString(UnicodeString const &crString) {
...
//    }
    ...
    CAS* CAS::getView(const UnicodeString &localViewName) {
        SofaID* sid;
    ...
    CAS* CAS::createView(UnicodeString const &localViewName) {
        // map the input name
        SofaID* sid;
        bool deleteSofaID = false;
        if (0 != iv_baseCas->iv_componentInfo) {
    ...
    void CAS::setSofaDataString(UnicodeStringRef const text, UnicodeString const &mimetype) {
        if (isbaseCas) {
            invalidBaseCasMethod();
        }
        SofaFS thisSofa = getSofa();
    ...
    void CAS::setSofaDataArray(FeatureStructure array, UnicodeString const &mime) {
        if (isbaseCas) {
            invalidBaseCasMethod();
        }
        SofaFS thisSofa = getSofa();
    ...
    void CAS::setSofaDataURI(UnicodeString const &uri, UnicodeString const &mime) {
        if (isbaseCas) {
            invalidBaseCasMethod();
        }
        if (!getSofa().isValid()) {
    ...
    bool CAS::moveToBeginPosition(ANIterator &itOfType, AnnotationFS const &crFsFromAn) {
        ...
        size_t uiBegPosFromAn = crFsFromAn.getIntValue(
                internal::FSPromoter::promoteFeature(internal::gs_tyBeginPosFeature, iv_heap->getTypeSystem()));
        ...
        while (itOfType.get().isValid()) {
            uiBegPosOfType = itOfType.get().getIntValue(
                    internal::FSPromoter::promoteFeature(internal::gs_tyBeginPosFeature, iv_heap->getTypeSystem()));
    ...
    */
    ...
    SofaFS CAS::createSofa(UnicodeStringRef const sofaName, UnicodeStringRef const mimeType) {
        ...
        bumpSofaCount();
        newSofa.setIntValue(
                internal::FSPromoter::promoteFeature(internal::gs_tySofaNumFeature, iv_heap->getTypeSystem()),
                iv_baseCas->iv_sofaCount);
        newSofa.setStringValue(
                internal::FSPromoter::promoteFeature(internal::gs_tySofaIDFeature, iv_heap->getTypeSystem()), sofaName);
        if (mimeType.length() > 0) {
            newSofa.setStringValue(
                    internal::FSPromoter::promoteFeature(internal::gs_tySofaMimeFeature, iv_heap->getTypeSystem()),
                    mimeType);
        }
    ...
    void CAS::bumpSofaCount() {
        ...
        if ((iv_baseCas->iv_sofaCount + 1) > (int) iv_baseCas->iv_sofa2indexMap.size()) {
            // enlarge map and set new contentEntry to NULL
...
//   }
    ...
    SofaFS CAS::createInitialSofa(UnicodeStringRef const mimeType) {
        ...
        SofaFS newSofa = (SofaFS) internal::FSPromoter::promoteFS(tySofa, *this->iv_baseCas);
        newSofa.setIntValue(
                internal::FSPromoter::promoteFeature(internal::gs_tySofaNumFeature, iv_heap->getTypeSystem()), 1);
        newSofa.setStringValue(
                internal::FSPromoter::promoteFeature(internal::gs_tySofaIDFeature, iv_heap->getTypeSystem()),
                NAME_DEFAULT_SOFA);
        newSofa.setStringValue(
                internal::FSPromoter::promoteFeature(internal::gs_tySofaMimeFeature, iv_heap->getTypeSystem()),
                mimeType);
    ...
    CAS &CAS::getCasForTyFS(lowlevel::TyHeapCell tyCell) {
        ...
        if (type != uima::lowlevel::FSHeap::INVALID_FS &&
            getHeap()->getTypeSystem().subsumes(internal::gs_tyAnnotationType,
                                                internal::FSPromoter::demoteType(
                                                        internal::FSPromoter::promoteFS(tyCell, *this).getType()))) {
    ...
    void SofaFS::setSofaMime(UnicodeString const &aString) {
        //TODO FAIL THIS IF MIME ALREADY SET ???
        lowlevel::TyFS tySofa = internal::FSPromoter::demoteFS(*this);
        this->setStringValue(internal::FSPromoter::promoteFeature(internal::gs_tySofaMimeFeature,
    ...
    void SofaFS::setRemoteSofaURI(UnicodeString const &aString) {
        //TODO NEED TO FAIL THIS IF DATA, STRING, or URI ALREADY SET !!!
        this->setStringValue(
                internal::FSPromoter::promoteFeature(internal::gs_tySofaURIFeature, iv_cas->getHeap()->getTypeSystem()),
                aString);
        return;
    }

    UnicodeStringRef SofaFS::getSofaMime() {
        return this->getStringValue(internal::FSPromoter::promoteFeature(internal::gs_tySofaMimeFeature,
                                                                         iv_cas->getHeap()->getTypeSystem()));
    }

    UnicodeStringRef SofaFS::getSofaID() {
        return this->getStringValue(
                internal::FSPromoter::promoteFeature(internal::gs_tySofaIDFeature, iv_cas->getHeap()->getTypeSystem()));
    }

    UnicodeStringRef SofaFS::getSofaURI() {
        return this->getStringValue(internal::FSPromoter::promoteFeature(internal::gs_tySofaURIFeature,
                                                                         iv_cas->getHeap()->getTypeSystem()));
    }

    int SofaFS::getSofaRef() {
        return this->getIntValue(internal::FSPromoter::promoteFeature(internal::gs_tySofaNumFeature,
                                                                      iv_cas->getHeap()->getTypeSystem()));
    }

    FeatureStructure SofaFS::getLocalFSData() {
        return this->getFSValue(internal::FSPromoter::promoteFeature(internal::gs_tySofaArrayFeature,
                                                                     iv_cas->getHeap()->getTypeSystem()));
    }

    UnicodeStringRef SofaFS::getLocalStringData() {
        return this->getStringValue(internal::FSPromoter::promoteFeature(internal::gs_tySofaStringFeature,
                                                                         iv_cas->getHeap()->getTypeSystem()));
    }
    ...
    AnnotationFS::AnnotationFS(FeatureStructure const &fs)
        ...
        assert(iv_cas->getHeap()->getTypeSystem().subsumes(
                iv_cas->getHeap()->getTypeSystem().getTypeByName(CAS::TYPE_NAME_ANNOTATION),
    ...
    namespace lowlevel {
        ...
        private:
        public:
            UnAmbiguousSubIterator(uima::lowlevel::FSHeap &heap, size_t uiBeginPos, size_t uiEndPos,
                                   IndexIterator* pIterator) :
            ...
            void moveToNext() {
                ...
                */
                ...
                size_t uiLastEndPos = (size_t) iv_heap->getIntValue(iv_pIterator->get(),
                                                                    uima::internal::gs_tyEndPosFeature);
                size_t uiCurrBeginPos;
                do {
                    iv_pIterator->moveToNext();
                    if (!isValid()) {
    ...
    ANIterator ANIndex::subIterator(AnnotationFS const &an, EnIteratorAmbiguity enAmbiguous) const {
        ...
        if (enAmbiguous == enUnambiguous) {
            pitSub = new lowlevel::UnAmbiguousSubIterator(iv_indexRepository->getFSHeap(),
                                                          internal::FSPromoter::demoteFS(an), pitBase);
        } else {
            assert(enAmbiguous == enAmbiguous);
            pitSub = new lowlevel::SubIterator(iv_indexRepository->getFSHeap(), internal::FSPromoter::demoteFS(an),
                                               pitBase);
        }
