...
namespace uima {
  ...
  void XMLWriterABase::normalize(UnicodeStringRef const & in, UnicodeString& out)  {
    //cout << "normalize() input " << in << endl;
    static UnicodeString const ustrAMP_ESC("&amp;");
    static UnicodeString const ustrGT_ESC("&gt;");
    static UnicodeString const ustrLT_ESC("&lt;");
    static UnicodeString const ustrAPOS_ESC("&apos;");
    static UnicodeString const ustrQUOT_ESC("&quot;");
    static UnicodeString const ustrCR_ESC("&#13;");
    static UnicodeString const ustrLF_ESC("&#10;");
  ...
  UnicodeString XMLDumpWriter::getTab(int iTab) const {
    UnicodeString usResult;
    UnicodeString usSpace(" ");
    int i;
    for (i=0; i<iTab; ++i) {
  ...
  void XMLDumpWriter::writeFS(int iTab, ostream& os, lowlevel::TyFS tyFS, bool bWriteShortStyle=false) const {
    UnicodeString usTab = getTab(iTab);
    ...
    if ( iv_addDocument && iv_rTypeSystem.subsumes( iv_tyAnnotationType, tyFSType ) ) {
      ...
      UnicodeString usNormalizedSpan;
    ...
    if (tyFSType == iv_tyStringType) {
      ...
      UnicodeString usNormalizedString;
    ...
    for (i=0; i<vecFeatures.size(); ++i) {
      ...
      UnicodeString newTab = getTab(iTab + TAB_INCREMENT);
  ...
  void XMLDumpWriter::writeArray(int iTab, ostream& os, lowlevel::TyFSFeature & tyFeat, lowlevel::TyFS & tyValue, lowlevel::TyFSType tyRange, UnicodeString & newTab) const {
    if (tyValue != 0) {
      os << newTab << "<"<< gs_cpszFeatTagName<<" name=\"" << iv_rTypeSystem.getFeatureName( tyFeat ) << "\"";
      os << ">" << endl;
      UnicodeString uiArrayTab = getTab( iTab + (2*TAB_INCREMENT) );
      UnicodeString uiArrayValTab = getTab( iTab + (3*TAB_INCREMENT) );
  ...
  void XMLDumpWriter::write(ostream& os) {
    ...
    vector<UnicodeString> vIndexIDs;

    indexDef.getAllIndexIDs(vIndexIDs);

    UnicodeString const & crAnnIndexID = iv_crCAS.getAnnotationIndexID();

    vector<UnicodeString>::iterator it;
    ...
    for (i=0; i<vIndexIDs.size(); ++i) {
      ...
      UnicodeString const & crIndexID = vIndexIDs[i];

      UnicodeString const & crTypeName = iv_rTypeSystem.getTypeName( indexDef.getTypeForIndex(crIndexID) );
...
//       }
    ...
    for (int view=0; view<=numViews; view++) {
      ...
      vector<UnicodeString> indexIDs = ixRep->getAllIndexIDs();
      size_t i;
