...
namespace uima {
  ...
  void XMLTypeSystemReader::checkValidityCondition(bool bCondition, TyMessageId tyMessage, UnicodeString const & crString) const {
    if (!bCondition) {
  ...
  void XMLTypeSystemReader::checkValidityCondition(bool bCondition, TyMessageId tyMessage, UnicodeString const & crString1, UnicodeString const & crString2) const {
    if (!bCondition) {
      uima::ErrorMessage msg(tyMessage);
  ...
  void XMLTypeSystemReader::createFeatures(DOMElement * pTopTypeElement) {
    ...
    for (i=0; i<featureList->getLength(); ++i) {
      ...
      UnicodeString rangeTypeName = convert( featureElement->getAttribute( convert(uima::internal::XMLConstants::ATTRIBUTENAME_RANGE) ) );
      UnicodeString featureName = convert( featureElement->getAttribute( convert(uima::internal::XMLConstants::ATTRIBUTENAME_NAME)) );
      UnicodeString multiRefs = convert( featureElement->getAttribute( convert(uima::internal::XMLConstants::ATTRIBUTENAME_MULTIREFS)) );
      ...
      UnicodeString introTypeName = convert( pIntroTypeElement->getAttribute(convert(uima::internal::XMLConstants::ATTRIBUTENAME_NAME)) );
      ...
      if (tyFeature != lowlevel::TypeSystem::INVALID_FEATURE) {
        ...
        // create the feature
		bool mr = (multiRefs == UnicodeString("true"));
  ...
  void XMLTypeSystemReader::createType(lowlevel::TyFSType tyParentType, DOMElement * pNewTypeElement) {
    ...
    if (tyParentType != lowlevel::TypeSystem::INVALID_TYPE) {
      ...
      // create the type
      UnicodeString newTypeName = convert( pNewTypeElement->getAttribute( convert(uima::internal::XMLConstants::ATTRIBUTENAME_NAME )) );
  ...
  void XMLTypeSystemReader::readMemory(UnicodeString const & xmlString, UnicodeString const & creatorID) {
  ...
  void XMLTypeSystemReader::readMemory(char const * cpszXMLString, UnicodeString const & creatorID) {
    MemBufInputSource memIS((XMLByte const *) cpszXMLString, strlen(cpszXMLString), "sysID");
    read(memIS, creatorID );
  }


  void XMLTypeSystemReader::readFile(char const * fileName, UnicodeString const & creatorID) {
  ...
  void XMLTypeSystemReader::readFile(UnicodeString const & fileName, UnicodeString const & creatorID) {
  ...
  void XMLTypeSystemReader::read(InputSource const & crInputSource, UnicodeString const & creatorID) {
    ...
    UnicodeString ustrTAGNAME_TYPEHIERARCHY(uima::internal::XMLConstants::TAGNAME_TYPEHIERARCHY);
    UnicodeString ustrTAGNAME_TYPE(uima::internal::XMLConstants::TAGNAME_TYPE);
    assertWithMsg(sizeof(XMLCh) == sizeof(UChar), "Port required");
    UnicodeString ustrRootName( (UChar const *) rootElem->getNodeName());
    ...
    while (i<children->getLength() ) {
      ...
      if ( kid->getNodeType() == DOMNode::ELEMENT_NODE ) {
        ...
        /* taph 02.10.2002: do we need to do the validity checking ourselves?
           ...
           And it is expensive because of the conversions. */
        assertWithMsg(sizeof(XMLCh) == sizeof(UChar), "Port required");
        UnicodeString ustrKidName((UChar const *) kidElem->getNodeName());
