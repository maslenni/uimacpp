...
using namespace std;
namespace uima {
  TypeNameSpace::TypeNameSpace(uima::TypeSystem const & crTypeSystem, UnicodeString const & crName)
      : iv_usName(crName),
      iv_cpTypeSystem( & crTypeSystem ) {}

  Type TypeNameSpace::getType(UnicodeString const & crTypeBaseName) const {
    UnicodeString typeName(iv_usName);
    typeName.append((UChar) TYPENAMESPACE_SEP);
    typeName.append(crTypeBaseName);
    return iv_cpTypeSystem->getType(typeName);
  }

  UnicodeString const & TypeNameSpace::getName() const {
    return iv_usName;
  }
  ...
  bool TypeNameSpaceImport::getType(UnicodeString const & crRelativeTypeName, Type & rResult) const {
    rResult = Type();
    size_t i;
