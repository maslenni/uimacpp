...
namespace uima {
  ...
  FSIndex FSIndexRepository::getIndex(UnicodeString const & crLabel) const {
    return getIndex(crLabel, getTypeForIndex(crLabel));
  }

  std::vector<UnicodeString> FSIndexRepository::getAllIndexIDs() const {
    vector<UnicodeString> ids;
    getLowlevelIndexRepository().getIndexDefinition().getAllIndexIDs(ids);
    return ids;
  }

  FSIndex FSIndexRepository::getIndex(UnicodeString const & crLabel, Type const & crType) const {
  ...
  Type FSIndexRepository::getTypeForIndex(UnicodeString const & crLabel) const {
  ...
  bool FSIndexRepository::isValidIndexID(UnicodeString const & crLabel) const {
    return getLowlevelIndexRepository().getIndexDefinition().isValidIndexId(crLabel);
  }

  bool FSIndexRepository::isValidIndexID(UnicodeString const & crLabel, Type const & crType) const {
    if (!isValidIndexID(crLabel) ) {
      return false;
    }
