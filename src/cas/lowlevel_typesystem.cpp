...
namespace uima {
  namespace lowlevel {
    UnicodeString const TypeSystem::ustrCREATOR_ID_SYSTEM("System");
...
namespace uima {
  namespace lowlevel {
    ...
    TyFSType TypeSystem::createType(TyFSType tyParent, UnicodeString const & crusName, UnicodeString const & crusCreatorID) {
      assert( !iv_bIsCommitted);
    ...
    TyFSType TypeSystem::createTypeNoChecks(TyFSType tyParent, UnicodeString const & crusName, UnicodeString const & crusCreatorID) {
    ...
    TyFSFeature TypeSystem::createFeature(TyFSType tyIntro, TyFSType tyRange, bool multiRef, UnicodeString const & crusName, UnicodeString const & crusCreatorID) {
    ...
    TyFSType TypeSystem::createType( StTypeInfo const & crTypeInfo, UnicodeString const & crusCreatorID) {
      UIMA_TPRINT("Adding type " << crTypeInfo.iv_cpszName << " as son of " << crTypeInfo.iv_cpszParentName);
      assert( !iv_bIsCommitted);
      TyFSType tyParent = getTypeByName(UnicodeString( crTypeInfo.iv_cpszParentName) );
    ...
    TyFSType TypeSystem::createStringSubtype(UnicodeString const & crName,
        vector<UnicodeString> const & crStrings,
        UnicodeString const & crusCreatorID) {
    ...
    TyFSFeature TypeSystem::createFeature( StFeatureInfo const & crFeatureInfo, UnicodeString const & crusCreatorID) {
      ...
      assert( !iv_bIsCommitted);
      TyFSType tyIntroType = getTypeByName(UnicodeString(crFeatureInfo.iv_cpszIntroTypeName) );
      assert( tyIntroType != INVALID_TYPE );
      TyFSType tyRangeType = getTypeByName(UnicodeString( crFeatureInfo.iv_cpszRangeTypeName) );
...
//         return findInTree(tyT1, tyT2);
    ...
    TyFSType TypeSystem::getTypeByName(UnicodeString const & crusTypeName) const {
    ...
    TyFSFeature TypeSystem::getFeatureByBaseName(TyFSType tyType, UnicodeString const & crusFeatureName) const {
    ...
    TyFSFeature TypeSystem::getFeatureByBaseName(UnicodeString const & crusTypeName, UnicodeString const & crusFeatureName) const {
    ...
    TyFSFeature TypeSystem::getFeatureByFullName(UnicodeString const & crusFeatureName) const {
    ...
    UnicodeString TypeSystem::getFeatureName(TyFSFeature tyFeature) const {

      UnicodeString const & crBaseName = getFeatureBaseName(tyFeature);
      UnicodeString const & crTypeName = getTypeName( getIntroType(tyFeature) );
      UnicodeString result( crTypeName );
    ...
    vector<UnicodeString> const & TypeSystem::getStringsForStringSubtype(TyFSType type) const {
...
//#define ASSERT_OR_RETURN_FALSE(x) if (!(x)) return false
    ...
    bool TypeSystem::debugIsTreeConsistent() const {
      ...
      for (i=1; i<iv_vecTypeNames.size(); ++i) {
        UnicodeString const & name = iv_vecTypeNames[i];
