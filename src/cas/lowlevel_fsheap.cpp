  ...
  namespace lowlevel {
    ...
    void FSHeap::setStringValue(TyFS tyFs, TyFSFeature tyFeature, int strRef) {
      ...
      if (rangeType != uima::internal::gs_tyStringType ) {
        vector<UnicodeString> const & crStringSet = iv_rclTypeSystem.getStringsForStringSubtype(rangeType);
        vector<UnicodeString>::const_iterator cit;
      ...
      if (tyCurrentString == 0) {
        // add contentEntry to StringRefHeap and set values there
        tyCurrentString = getStringAsFS(strRef);
      } else {
        // reuse contentEntry in StringRefHeap
...
#pragma warning( disable : 4244 )
    ...
    void FSHeap::printFS(ostream& os, TyFS fs) const {
      ...
      UnicodeString typeName = iv_rclTypeSystem.getTypeName(type);
      int n=0;
      os << n++ << ":  | " << typeName << endl;
      unsigned int i;
