...
// Type
  ...
  Feature Type::getFeatureByBaseName(UnicodeString const & crBaseName) const {
    checkValidity();
...
//      crOther.checkValidity();
  ...
  Type TypeSystem::getType(UnicodeString const & crTypeName) const {
    return internal::FSPromoter::promoteType( getLowlevelTypeSystem().getTypeByName( crTypeName ), getLowlevelTypeSystem() );
  }


  Feature TypeSystem::getFeatureByFullName(UnicodeString const & crFeatureName) const {
