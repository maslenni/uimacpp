...
#define USED_JAVA_VERSION JNI_VERSION_1_2
    ...
    // not NLS enabled !!!!!!!
    UnicodeString javaErrorToString(jint res) {
      switch (res) {
      case JNI_OK:
        return "no error";
      case JNI_ERR:
    ...
    void JEDIIEngine::checkForUnexpectedJNIException() {
      ...
      if (exc != NULL) {
        ...
        UnicodeString usMsg((UChar const *)jc, len);
        ...
        if ( msg != NULL ) {
          ...
          usMsg.append( UnicodeString((UChar const *)jc, len) );
          iv_env->ReleaseStringChars(msg, jc);
        }
    ...
    void JEDIIEngine::createStringVector(jobjectArray ar, vector<UnicodeStringRef>& result, vector<UnicodeString> & pool) {
      pool.clear();
      result.clear();
      jsize len = iv_env->GetArrayLength(ar);
      checkForUnexpectedJNIException();
      ...
      for (i=0; i<len; ++i) {
        ...
        if (js != NULL) {
          ...
          checkForUnexpectedJNIException();
          UnicodeString  us((UChar const *) jc, (size_t) strLen);
          iv_env->ReleaseStringChars(js, jc);
          checkForUnexpectedJNIException();
          pool.push_back(us);

          UnicodeString const & poolString = pool.back();
    ...
    void JEDIIEngine::handleEngineProxyError(jint rc) {
      if (rc != JNI_OK) {
        ...
        checkForUnexpectedJNIException();
        UnicodeString  us((UChar const *) jc, (size_t) js);
    ...
    void JEDIIEngine::handleJNIError(jint rc, UnicodeString const & us) {
      if (rc != JNI_OK) {
...
//         ofstream ofs("jedii_tmp_specifier.xml"); ofs << config << endl; ofs.close();
      ...
      if (versionString != NULL) {
        ...
        checkForUnexpectedJNIException();
        UnicodeString  uVersion((UChar const *) jc, (size_t) js);
        cout << uVersion << endl;
      ...
      UIMA_TPRINT("definitions serialized");
      {
        ...
        UnicodeString dataPathUS(uima::ResourceManager::getInstance().getLocationData().getAsCString() );
...
//         serializer.serializeData( TCAS::promoteCAS(cas), serializedInCAS);
    ...
    void JEDIIEngine::destroyJavaVM() {
      ...
      if (iv_createdVM) {
        ...
        if (destroyRC != JNI_OK) {
          char c[2056];
          UnicodeString us = javaErrorToString(destroyRC);
