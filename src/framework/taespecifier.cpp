...
namespace uima {
  ...
  void AnalysisEngineMetaData::validate() {
    ...
    vector <UnicodeString> indexLabels;
    ...
    const vector <UnicodeString> groupNames = getConfigurationGroupNames();
  ...
  * the group again.
  **/
  TyErrorId AnalysisEngineMetaData::setDefaultGroupName(const UnicodeString & groupName) {
    if (! isModifiable()) {
  ...
  TyErrorId AnalysisEngineMetaData::addConfigurationGroup(const UnicodeString & groupName) {
    if (! isModifiable()) {
      return UIMA_ERR_CONFIG_OBJECT_COMITTED;
    }
  ...
  TyErrorId AnalysisEngineMetaData::addConfigurationParameter(const UnicodeString & groupName,
  ...
  void AnalysisEngineMetaData::addNameValuePair(const UnicodeString & groupName,
      NameValuePair * nvPair) {
    if (! isParameterDefined(groupName, nvPair->getName(), "")) {
      const UnicodeString & paramName = nvPair->getName();
      if (hasGroups()) {
  ...
  NameValuePair * AnalysisEngineMetaData::getNameValuePairPtr(const UnicodeString & groupName,
  ...
  const UnicodeString & AnalysisEngineMetaData::getGroupNameWhenNotSpec() const {
    if (hasGroups()) {
      if (hasDefaultGroup()) {
  ...
  **/
  NameValuePair * AnalysisEngineMetaData::getNameValuePair(const UnicodeString & paramName,
      const UnicodeString & ancKey) {
    return getNameValuePair(getGroupNameWhenNotSpec(), paramName, getSearchStrategy(), ancKey);
  }

  NameValuePair *  AnalysisEngineMetaData::getNameValuePair(const UnicodeString & groupName,
      const UnicodeString & paramName,
    ...
    vector < UnicodeString> fallbackGroups;
  ...
  NameValuePair * AnalysisEngineMetaData::getNameValuePairNoFallback(const UnicodeString & groupName,
      const UnicodeString & paramName,
      const UnicodeString & ancKey) {
    //sanity check
    if (! hasGroups()
  ...
  TyErrorId AnalysisEngineMetaData::setNameValuePair(const UnicodeString & groupName,
  ...
  void AnalysisEngineMetaData::generateFallbackGroups(const UnicodeString & groupName,
      EnSearchStrategy strategy,
      vector < UnicodeString> & fallbackGroups) {
    int i;
    bool cutOffPossible;
  ...
  **/
  bool AnalysisEngineMetaData::isParameterDefined(const UnicodeString & paramName,
  ...
  **/
  bool AnalysisEngineMetaData::isParameterDefined(const UnicodeString & groupName,
      const UnicodeString & paramName,
      const UnicodeString & ancKey) const {
  ...
  const vector <UnicodeString> AnalysisEngineMetaData::getConfigurationGroupNames() const {
    vector <UnicodeString> groupNames;
    TyConfigGroup::const_iterator ite;
    for (ite = iv_configurationGroups.begin(); ite != iv_configurationGroups.end(); ite++) {
      const UnicodeString & groupName = ite->first;
  ...
  const vector <const ConfigurationParameter *> AnalysisEngineMetaData::getConfigurationParameters(const UnicodeString & groupName) const {
  ...
  const vector <UnicodeString > AnalysisEngineMetaData::getGroupNamesForParameter(const UnicodeString & paramName) const {
    vector <UnicodeString > groups;
    vector <UnicodeString> allGroups = getConfigurationGroupNames();
    vector <UnicodeString>::const_iterator ite;
  ...
  /*-------------------------------------------------------*/
  AnalysisEngineDescription * AnalysisEngineDescription::getDelegate(const UnicodeString & key) const {
  ...
  AnalysisEngineDescription * AnalysisEngineDescription::extractDelegate(const UnicodeString & key) {
    TyMapDelegateSpecs::iterator ite = iv_pMapDelegateSpecifiers->find(key);
    if (ite == iv_pMapDelegateSpecifiers->end()) {
      return NULL;
    } else {
  ...
  void AnalysisEngineDescription::toXMLBuffer(UnicodeString & s) const {
  ...
  void appendCapabilities(Capability::TyVecCapabilityTofs const & v, char const * xmlTag, UnicodeString & s) {
    size_t i;
    for (i=0; i<v.size(); ++i) {
      s.append("<");
      s.append(xmlTag);
  ...
  void appendSofaName(Capability::TyVecCapabilitySofas const & v, char const * xmlTag, UnicodeString & s) {
    size_t i;
    for (i=0; i<v.size(); ++i) {
      s.append("<");
      s.append(xmlTag);
  ...
  void appendBool(bool b, UnicodeString & s) {
    if (b) {
      s.append("true");
    } else {
      s.append("false");
  ...
  void appendFloat(float f, UnicodeString & s) {
    char c[256];
    sprintf(c, "%f", f);
    s.append(c);
  }

  void appendInt(int i, UnicodeString & s) {
    char c[256];
    sprintf(c, "%d", i);
    s.append(c);
  }
  ...
  void AnalysisEngineDescription::appendConfigGroupToXMLBuffer(ConfigurationGroup const & config, UnicodeString & s) const {
    size_t i;
  ...
  void AnalysisEngineDescription::appendConfigGroupSettingsToXMLBuffer(SettingsForGroup const & settings, UnicodeString & s) const {
    size_t i,j;
    ...
    for (i=0; i<nvps.size(); ++i) {
      ...
      if (nvp->isMultiValued()) {
        ...
        case ConfigurationParameter::STRING: {
            vector<UnicodeString> const & v = nvp->extractStringValues();
            for (j=0; j<v.size(); ++j) {
              s.append("<string>");
              s.append(v[j]);
              s.append("</string>");
        ...
        case ConfigurationParameter::STRING: {
            UnicodeString const & v = nvp->extractStringValue();
            s.append("<string>");
            s.append(v);
            s.append("</string>");
            break;
   ...
   UnicodeString & s) {
    ...
    if (prioDesc.size() > 0) {
      ...
      for (i=0; i<prioDesc.size() ;++i) {
        ...
        vector <UnicodeString> vecTypeOrder = prio->getTypeOrder();
  ...
  void AnalysisEngineDescription::appendConfigParamsAndSettingsToXMLBuffer(UnicodeString & s) const  {
    ...
    if (md.hasGroups()) {
      ...
      for ( cit = confGroups.begin(); cit != confGroups.end(); ++cit) {
        UnicodeString const & confGroupName = (*cit).first;
    ...
    if (md.hasGroups()) {
      ...
      for (cit = confSettings.begin(); cit != confSettings.end(); ++cit) {
        UnicodeString const & groupName = (*cit).first;
  ...
  void AnalysisEngineDescription::toXMLBufferNoXMLHeader(UnicodeString & s) const {
    size_t i;
  ...
  void AnalysisEngineDescription::toXMLBuffer(AnalysisEngineMetaData const & md,  
    ...
    if (!isPrimitive()) {
      ...
      vector<UnicodeString> const & v = flow->getNodes();
      for (i=0; i<v.size(); ++i) {
        s.append("<node>");
        s.append(v[i]);
        s.append("</node>");
