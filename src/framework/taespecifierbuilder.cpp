...
namespace uima {
  ...
  void XMLParser::setErrorHandler(ErrorHandler * pErrorHandler) {
   ...
   void XMLParser::parseAnalysisEngineDescription(AnalysisEngineDescription & aeSpec,
    ...
    UnicodeString ustrFileName(fileName);
  ...
  void XMLParser::parseAnalysisEngineDescription(AnalysisEngineDescription & taeSpec,
    ...
    UnicodeString const & fn = ResourceManager::resolveFilename(fileName, ".");
  ...
  void XMLParser::buildAnalysisEngineDescription(AnalysisEngineDescription & taeSpec,
      DOMElement * descElem,
      const UnicodeString & xmlFileLoc) {
    assert(EXISTS(descElem));
    ...
    try {
      ...
      for (i=0; i < children->getLength(); i++) {
        ...
        const UnicodeString & childTag = convert((children->item(i))->getNodeName());
        if (childTag.compare(TAG_TAE_PRIMITIVE) == 0) {
          ...
          if (EXISTS(delegateAEs)) {
            ...
            for (j=0; j < delegateAEs->getLength(); j++) {
              ...
              UnicodeString key = convert(delegateAE->getAttribute(convert(ATTR_DELEGATE_AE_KEY)));
              ...
              if (EXISTS(delegateSpec)) {           //the delegate is explicitly copied into the aggregate xml file
                ...
                if (delegateSpec != NULL) {
                  UnicodeString loc = convert(delegateSpec->getAttribute(convert(ATTR_IMPORT_DESC_LOCATION)));
				  UnicodeString fn = ResourceManager::resolveFilename(loc, xmlFileLoc);
				  if (loc.length() > 0) {			    
                  ...
                  if (EXISTS(delegateInc)) {
                    UnicodeString newFileName = ResourceManager::resolveFilename(getSpannedText(delegateInc),
                                                     xmlFileLoc);
          ...
          std::vector<UnicodeString> alreadyImportedTypeSystemLocations;
          ...
          const UnicodeString & impName = getSpannedText(children->item(i));
  ...
  AnalysisEngineMetaData * XMLParser::buildAEMetaData(DOMElement * descElem,
      UnicodeString const & xmlFileLoc) {
    assert(EXISTS(descElem));
    ...
    for (i=0; i < children->getLength(); i++) {
      ...
      const UnicodeString & childTag = convert(child->getNodeName());
      if (childTag.compare(TAG_AE_NAME) == 0) {
        ...
        vector <UnicodeString> alreadyImported;
  ...
  void XMLParser::buildConfigParams(AnalysisEngineMetaData & aeMetaData,
    ...
    if (isDefined(searchStrategy)) {
      const UnicodeString & strategy = convert(searchStrategy);
      if (strategy.compare(NO_FALLBACK) == 0) {
        ...
        UnicodeString tagName = convert( descElem->getTagName() );
    ...
    for (i=0; i < children->getLength(); i++) {
      ...
      if (childTag.compare(TAG_CONFIG_PARAM_COMMON) == 0) {
        ...
        const UnicodeString & groupNameString = convert(child->getAttribute(convert(ATTR_CONFIG_PARAM_GROUP_NAMES)));
        ...
        for (size_t m=0; m < groupNames.size(); m++) {
          const UnicodeString & groupName = UnicodeString(groupNames[m].c_str(), "UTF-8");
        ...
        for (k=0; k < confParmsInGroup->getLength(); k++) {
          ...
          for (l=0; l < groupNames.size(); l++) {
            ...
            aeMetaData.addConfigurationParameter(UnicodeString(groupNames[l].c_str(), "UTF-8"), configParam);
          }
        }
      }
    }
  ...
  ConfigurationParameter * XMLParser::buildConfigParam(DOMElement * descElem) {
    ...
    for (i=0; i < children->getLength(); i++) {
      ...
      if (childTag.compare(TAG_CONFIG_PARAM_NAME) == 0) {
        ...
        const UnicodeString & typeName = getSpannedText(child);
        ...
        const UnicodeString & ancKeyString = getSpannedText(child);
        ...
        for (size_t i=0; i < ancKeys.size(); i++) {
          const UnicodeString & key = UnicodeString(ancKeys[i].c_str(), "UTF-8");
  ...
  void XMLParser::buildConfigParamSettings(AnalysisEngineMetaData & aeMetadata,
    ...
    for (i=0; i < children->getLength(); i++) {
      ...
      if (childTag.compare(TAG_CONFIG_PARAM_SETTING_GROUP) == 0) {
        ...
        const UnicodeString & groupNameString = convert(child->getAttribute(convert(ATTR_CONFIG_PARAM_SETTING_GROUP_NAMES)));
        vector <std::string> groupNames;
        //TBD: change this to true unicode processing
        delimitedString2Vector(groupNames,
                               UnicodeStringRef(groupNameString).asUTF8(),
        ...
        for (k=0; k < nvPairsForGroup->getLength(); k++) {
          ...
          for (l=0; l < groupNames.size(); l++) {
            ...
            UnicodeString groupName(groupNames[l].c_str(), "UTF-8");
  ...
  ConfigurationParameter::EnParameterType XMLParser::findTypeForValue(DOMNode * descElem) {
    assert(EXISTS(descElem));
    const UnicodeString & tagName = convert(descElem->getNodeName());
  ...
  void XMLParser::buildTypePriorities(AnalysisEngineMetaData & aeMetaData,
      DOMElement * descElem,
      UnicodeString const & xmlFileLoc,
      vector<UnicodeString> & alreadyImportedLocations) {
  ...
  FSIndexDescription * XMLParser::buildFSIndexDesc(DOMElement * descElem) {
    ...
    for (i=0; i< children->getLength(); i++) {
      ...
      if (childTag.compare(TAG_FS_INDEX_DESC_LABEL) == 0) {
        ...
        const UnicodeString & kind = getSpannedText(child);
  ...
  FSIndexKeyDescription * XMLParser::buildFSIndexKeyDesc(DOMElement * descElem) {
    ...
    for (i=0; i < children->getLength(); i++) {
      ...
      if (childTag.compare(TAG_FS_INDEX_KEY_FEAT) == 0) {
        ...
        const UnicodeString & compType = getSpannedText(child);
    ...
    for (i=0; i < children->getLength(); i++) {
      ...
      if (childTag.compare(TAG_CAP_LANG_SUPP) == 0) {
        ...
        for (j=0; j < types->getLength(); j++) {
          ...
          const UnicodeString & nodeTag = convert(nodeElem->getNodeName());
          ...
          if (nodeTag.compare(TAG_CAP_TYPE) == 0) {
            ...
            if (expand) {
              ...
              UnicodeString typeName(getSpannedText(nodeElem));
...
void XMLParser::buildFSIndexFromImportLocation(AnalysisEngineMetaData& fsDesc,
  ...
  TypeSystemDescription * TextAnalysisEngineSpecifierBuilder::buildTypeSystemSpecifierFromMemory(UnicodeString const & xmlString) {
  ...
  //----------------------------------------------------
  bool XMLParser::isTrue(const UnicodeString & value) const {
  ...
  UnicodeString XMLParser::getSpannedText(DOMNode * node) {
    assert(EXISTS(node));
    DOMNodeList * children = node->getChildNodes();
    assert(EXISTS(children));
    UnicodeString spannedText;
    size_t i;
    for (i=0; i < children->getLength(); i++) {
      if (children->item(i)->getNodeType() == DOMNode::TEXT_NODE) {
        ...
        if (childTag.compare(TAG_ENV_VAR_REF) == 0) {
          UnicodeString envVarName(convert(children->item(i)->getFirstChild()->getNodeValue()));
