...
#define CTX_DEFAULT_TEXT_SOFA_NAME  "_DefaultTextSofaName"
    ...
    for (iter = vecSofaMappings.begin(); iter != vecSofaMappings.end(); ++iter) {
      ...
      if (sofamap->getComponentKey().compare(ancKey) == 0) {
        ...
        if (sofamap->getComponentSofaName().length() == 0) {
          UnicodeString ustr(CAS::NAME_DEFAULT_TEXT_SOFA);
        ...
        UnicodeString astr = sofamap->getComponentSofaName();
  ...
  AnnotatorContext * AnnotatorContext::getDelegate(const UnicodeString & key) const {
  ...
  AnnotatorContext * AnnotatorContext::extractDelegate(const UnicodeString & key) {
  ...
  TyErrorId AnnotatorContext::assignValue(const UnicodeString & paramName,
                                          const bool & value) {
  ...
  TyErrorId AnnotatorContext::assignValue(const UnicodeString & paramName,
                                          const vector< UnicodeString > & values) {
    NameValuePair nvPair;
    nvPair.setName(paramName);
  ...
  TyErrorId AnnotatorContext::assignValue(const UnicodeString & groupName,
  ...
  NameValuePair const *
  AnnotatorContext::findNameValuePair(const UnicodeString & groupName,
                                      const UnicodeString & paramName,
  ...
  **/
  NameValuePair const * AnnotatorContext::findNameValuePair(const UnicodeString & paramName,
      const UnicodeString & ancKey) const {
  ...
  TyErrorId AnnotatorContext::setNameValuePair(const UnicodeString & groupName,
  ...
  void AnnotatorContext::buildDelegateAnCs() {
    ...
    vector < UnicodeString > delegateNames;
  ...
  bool AnnotatorContext::isParameterDefined(const UnicodeString & paramName) const {
  ...
  bool AnnotatorContext::isParameterDefined(const UnicodeString & groupName,
  ...
  const set <UnicodeString> AnnotatorContext::getGroupNamesForParameter(const UnicodeString & paramName) const {
      set <UnicodeString> groupNames;
      AnalysisEngineMetaData const * pMetaData = getTaeSpecifier().getAnalysisEngineMetaData();
      ...
      vector <UnicodeString> localGroups = pMetaData->getGroupNamesForParameter(paramName);
