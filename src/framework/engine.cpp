...
namespace uima {
...
#else
        string* pstrRetVal = new string(
                _TEXT("??? UNKNOWN UIMACPP ERROR ID:")); /* this memory gets wasted - but this is only NDEBUG code!!! */
        string strValue;

        long2String((long) utErrorId, strValue);
        assert(EXISTS(pstrRetVal));
        *pstrRetVal += strValue;
        *pstrRetVal += _TEXT(" - Please update file '"
                                     __FILE__
                                     "' ???");
    ...
    /*static*/ AnalysisEngine* Framework::createAnalysisEngine(AnnotatorContext& rANC,
        ...
        try {
            ...
            if (crTAESpecifier.getFrameworkImplName() == AnalysisEngineDescription::JAVA) {
                pResult = new uima::internal::JEDIIEngine(rANC, bOwnsANC, bOwnsTAESpecifier, casDefinition,
                                                          ownsCASDefintion);
            } else {
                if (crTAESpecifier.isPrimitive()) {
                    pResult = new uima::internal::PrimitiveEngine(rANC, bOwnsANC, bOwnsTAESpecifier, casDefinition,
                                                                  ownsCASDefintion);
                } else {
                    pResult = new uima::internal::AggregateEngine(rANC, bOwnsANC, bOwnsTAESpecifier, casDefinition,
                                                                  ownsCASDefintion);
                }
            }
    ...
    AnalysisEngine* Framework::createAnalysisEngine(bool bIsFilename,
        ...
        try {
            ...
            */
            ...
            auto_ptr<uima::internal::CASDefinition> apCASDef(
    ...
    TextAnalysisEngine* TextAnalysisEngine::createTextAnalysisEngine(char const* cpFileName, ErrorInfo& rErrorInfo) {
        UnicodeString usFileName(cpFileName);
    ...
    /*static*/
    TextAnalysisEngine* TextAnalysisEngine::createTextAnalysisEngine(UChar const* cpBuffer, size_t uiLength,
                                                                     ErrorInfo& rErrorInfo) {
    ...
    AnalysisEngine* Framework::createAnalysisEngine(UChar const* cpBuffer,
        ...
        UnicodeString usXMLBuffer(false, cpBuffer, uiLength);
    ...
    CAS* Framework::createCAS(TypeSystem& typesystem, ErrorInfo& rErrorInfo) {
        auto_ptr<uima::internal::CASDefinition> apCASDef(
                uima::internal::CASDefinition::createCASDefinition(typesystem));
    ...
    CAS* Framework::createCAS(TypeSystem& typesystem, AnalysisEngineMetaData& aeDesc, ErrorInfo& rErrorInfo) {
        auto_ptr<uima::internal::CASDefinition> apCASDef(
                uima::internal::CASDefinition::createCASDefinition(typesystem, aeDesc));
        if (apCASDef.get() == NULL) {
            rErrorInfo.setErrorId(UIMA_ERR_ENGINE_OUT_OF_MEMORY);
            return NULL;
        }
