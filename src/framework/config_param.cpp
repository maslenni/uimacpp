...
namespace uima {
  ...
  ConfigurationGroup::~ConfigurationGroup() {
    map<UnicodeString, ConfigurationParameter *>::iterator entries = iv_configParams.begin();
  ...
  const vector <const ConfigurationParameter *> ConfigurationGroup::getConfigurationParameters() const {
    ...
    map<UnicodeString, ConfigurationParameter *>::const_iterator ite;
  ...
  bool ConfigurationParameter::isDefinedForAnnotatorContext(const UnicodeString & ancKey) const {
  ...
  vector<NameValuePair const *> SettingsForGroup::getNameValuePairs() const {
    ...
    map<UnicodeString, NameValuePair *>::const_iterator ite;
  ...
  void NameValuePair::reset() {
    ...
    if (!iv_enAggregation) {
      ...
      case ConfigurationParameter::STRING  :
        iv_strValues.push_back(UnicodeString());
        break;
  ...
  TyErrorId NameValuePair::setValue(const UnicodeString & value) {
    ensureSingleValue();
    switch (iv_enType) {
    case ConfigurationParameter::STRING  :
  ...
  TyErrorId NameValuePair::addValue(const UnicodeString & value) {
    ensureMultipleValues();
    switch (iv_enType) {
    case ConfigurationParameter::STRING  :
  ...
  UnicodeString const & NameValuePair::extractStringValue() const {
    ensureType(ConfigurationParameter::STRING);
    ensureSingleValue();
    return(iv_strValues[0]);
  }

  vector<UnicodeString> const & NameValuePair::extractStringValues() const {
    ensureType(ConfigurationParameter::STRING);
    ensureMultipleValues();
    return(iv_strValues);
  }
  ...
  /* inline */ std::string NameValuePair::convert(const UnicodeString & value) const {
