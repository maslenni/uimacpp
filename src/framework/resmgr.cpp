...
namespace uima {
    ...
    /* ----------------------------------------------------------------------- */
    {
        ...
        if (clEnvVarSchemaPath.hasValue()) {
            ...
            if (bIsSchemaAvailable) {
                UnicodeString schemaPath(schemaFilename.getAsCString());
    ...
    /* ----------------------------------------------------------------------- */
    {
        ...
        UnicodeString const &crAnnotatorKind = annotatorFactory.getKind();
    ...
    /* ----------------------------------------------------------------------- */
    {
        ...
        UnicodeString us(crclFilename.getAsCString());
        uima::ErrorInfo errInfo;
        internal::ResourceAnnotatorFile const* cpResAnnotatorFile = (internal::ResourceAnnotatorFile const*) getResource(
        us, factory, errInfo);
    ...
    ResourceABase const* ResourceManager::getResource(uima::Language const &crLang,
                                                      ...
                                                      ErrorInfo &rErrInfo) {
        UnicodeString us(crLang.asString().c_str());
        return getResource(us, crFactory, rErrInfo);
    }

    ResourceABase const* ResourceManager::getResource(UnicodeString const &crKey,
                                                      ResourceFactoryABase const &crFactory,
                                                      uima::ErrorInfo &rErrInfo) {
//      assertWithMsg(false, "Implement caching functionality");
        UnicodeString const &crKind = crFactory.getKind();
    ...
    void ResourceManager::registerFactory(UnicodeString const &crKind, ResourceFactoryABase &crFactory) {
        iv_resourceFactories.insert(TyResourceFactories::value_type(crKind, &crFactory));
    }

    void ResourceManager::deRegisterFactory(UnicodeString const &crKind, ResourceFactoryABase &crFactory) {
    ...
    UnicodeString const &crKey,
    UnicodeString const &crKind,
    ErrorInfo &errorInfo) {
        TyResourceFactories::iterator it = iv_resourceFactories.find(crKind);
        if (it == iv_resourceFactories.end()) {
    ...
    /* static */ ResourceManager &ResourceManager::createInstance(const TCHAR* cpszInstance,
                                                                  const TCHAR* cpszProductPrefix)
    /* ----------------------------------------------------------------------- */
    {
        // acquire mutex
        if (NOTEXISTS(cv_pclSingletonInstance)) {
            ...
            cv_pclSingletonInstance->iv_frameworkLogger = new LogFacility(UnicodeString("org.apache.uima.cpp"),
                                                                          cv_pclSingletonInstance->iv_logLevel);
...
//#endif
    ...
    ResourceManager::createFilenameForLanguage(Language &rclLanguage,
        ...
        UIMA_TPRINT(
        _TEXT("1st Filename: '") << clFilename.getAsCString() << _TEXT("' existent: ") << clFilename.isExistent());
        if (clFilename.isExistent()) {
            ...
            if (rclLanguage.hasTerritory()) {
                ...
                UIMA_TPRINT(_TEXT("2nd Filename: '") << clFilename.getAsCString() << _TEXT("' existent: ") <<
                            clFilename.isExistent());
            }
        }
        if (bUseAlternateTerritories && !clFilename.isExistent()) {
            ...
            UIMA_TPRINT(
            _TEXT("3rd Filename: '") << clFilename.getAsCString() << _TEXT("' existent: ") << clFilename.isExistent());
        }
        rclFilename = clFilename;
    ...
    UnicodeString ResourceManager::resolveFilename(UnicodeString const &filename, UnicodeString const &lastFilename) {
        ...
        if (fileInDataDir.isExistent()) {
            return UnicodeString(fileInDataDir.getAsCString());
        }
