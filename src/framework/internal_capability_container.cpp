...
namespace uima {
  ...
  namespace internal {
    ...
    CapabilityContainer::hasEmptyOutputTypeOrFeatures( Language const & lang) const {
      ...
      TySetTypeOrFeatures const * pSetTOFs = findOutputTOFs(lang);
      if (pSetTOFs == NULL) {
        // we must have an explicit contentEntry for that language in our map
        // or maybe we have an empty contentEntry for the unspecifed lang
        return iv_bHasEmptyOutputTOFsForUnspecifiedLang;
      }
      // but that explicit contentEntry must be empty
      if (pSetTOFs->size() > 0) {
        return false;
      }
      return true;
