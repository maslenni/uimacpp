...
static UnicodeString ustrCreatorID("test");
...
namespace uima {
    namespace internal {
        ...
        */
                ...
                UnicodeString typesystemFileUnicode(typesystemFile.getAsCString());
                UnicodeString indexesFileUnicode(indexesFile.getAsCString());

                // add the typesystem from an xml file
                std::string tt_types(
                        "<?xml version=\"1.0\"?> <taeDescription><analysisEngineMetaData> <typeSystemDescription> <imports> <import location=\"");
                tt_types.append(typesystemFile.getAsCString());

                tt_types.append(
                        "\"/> </imports> </typeSystemDescription> <fsIndexCollection> <imports> <import location=\"");
                ...
                //         UnicodeString fileLoc( typesystemFile.getAsCString() );
...
//   FSHeap heap(ts, 50000);
    ...
    UnicodeString usAnnotationTypeName(CAS::TYPE_NAME_ANNOTATION);
    ...
    lowlevel::TyFSFeature lemmaEntryFeat = ts.getFeatureByBaseName(tokenAnnotationType,
                                                                   TT::FEATURE_BASE_NAME_LEMMA_ENTRIES);
...
void checkForwardOrder(lowlevel::FSHeap const& heap, auto_ptr<uima::lowlevel::IndexIterator>& it,
                       uima::lowlevel::IndexComparator const* cpComp, vector<lowlevel::TyFS> const& fsVec) {
    size_t i = 0;
...
void checkReverseOrder(lowlevel::FSHeap const& heap, auto_ptr<uima::lowlevel::IndexIterator>& it,
                       uima::lowlevel::IndexComparator const* cpComp, vector<lowlevel::TyFS> const& fsVec) {
    size_t i = 0;
    vector<lowlevel::TyFS> retrievedFSs;
...
void checkPreviousNextMovements(auto_ptr<uima::lowlevel::IndexIterator>& it,
                                uima::lowlevel::IndexComparator const* cpComp, vector<lowlevel::TyFS> const& fsVec) {
    size_t i = 0;
    vector<lowlevel::TyFS> retrievedFSs;
    for (it->moveToLast(); it->isValid(); it->moveToPrevious()) {
...
//   lowlevel::IndexComparator lemmaComp(heap, lemmaType, keyFeatures);
    ...
    // create indexes
    const UnicodeString ORDEREDIX = "42";
    const UnicodeString SETIX = "37";
    const UnicodeString FIFOIX = "4711";
...
//#define LEMMA_NUM 5
    ...
    vector<UnicodeString> lemmaStrings;
...
//      uima::internal::TCASImpl tcasimpl(ttdef, 5000, 5000, 5000);
    ...
    for (i = 0; i < (int) LEMMA_NUM; ++i) {
        ...
        UnicodeString const& lemmaKey = lemmaStrings[lemmasTobeCreated[i].keyIndex];
        ...
        heap.setFSValue(lemmaList, heap.getTypeSystem().getFeatureByBaseName(CAS::TYPE_NAME_NON_EMPTY_FS_LIST,
                                                                             CAS::FEATURE_BASE_NAME_TAIL), eList);
        heap.setFSValue(lemmaList, heap.getTypeSystem().getFeatureByBaseName(CAS::TYPE_NAME_NON_EMPTY_FS_LIST,
                                                                             CAS::FEATURE_BASE_NAME_HEAD), fs);
...
//      ensure that duplicates of lemmas are not inserted
    ...
    for (i = 0; i < (int) lemmas.size(); ++i) {
        lowlevel::TyFS lemma = heap.getFSValue(lemmas[i], heap.getTypeSystem().getFeatureByBaseName(
                CAS::TYPE_NAME_NON_EMPTY_FS_LIST, CAS::FEATURE_BASE_NAME_HEAD));
    ...
    // 4. test delete index
    UnicodeString anIxID = tcasimpl.getAnnotationIndexID();
...
//      uima::internal::TCASImpl tcas(ttdef, 50000, 50000, 50000);
  ...
  */
    ...
    UnicodeString const& crIndexID = tcas.getAnnotationIndexID();
    try {
...
void testOOFilterBuilder() {
    ...
    lolTS.createFeature(lolMyTokenType, uima::internal::FSPromoter::demoteType(stringType), false, "MyStringFeature",
                        ustrCreatorID);
...
//      uima::internal::TCASImpl tcas(ttdef, 4000, 4000, 4000);
    ...
    for (i = 0; i < tokenNum; ++i) {
        UnicodeString typeName = lolTS.getTypeName(tokens[i].annot.type);
        ...
        if (type == myTokenType) {
            ...
            UnicodeString us("string");
            us.append(UnicodeString(c));
//         UnicodeString icuus( (UChar*) us.data(), us.length() );
...
//   FSFilter * matchFilter2 = builder.createMatchFilter(vFeaturePath, stringFilter);
    ...
    for (ixit.moveToFirst(); ixit.isValid(); ixit.moveToNext()) {
        ...
        if (fs.getType() == myTokenType) {
            ...
            UnicodeString str(p.getBuffer(), p.length());
...
void testIteratorSetToPosition() {
    ...
    UnicodeString const& ixID = tcas.getAnnotationIndexID();
    ...
    LOG("Setting it2 to point to token from " << fsHeap.getIntValue(it1->get(), beginFeature) << " to " <<
        fsHeap.getIntValue(it1->get(), endFeature));
    ...
    for (i = numberOfAdvances; i < tokenNum; ++i) {
        ASSERT_OR_THROWEXCEPTION(it1->isValid());
        ASSERT_OR_THROWEXCEPTION(it2->isValid());
        LOG("Checking token from " << fsHeap.getIntValue(it1->get(), beginFeature) << " to " <<
            fsHeap.getIntValue(it1->get(), endFeature));
...
//      uima::internal::TCASImpl tcas(ttdef, 4000, 4000, 4000);
  ...
  UnicodeString const & annotationIndexID = tcas.getAnnotationIndexID();
...
void testOOStringArrayFS() {
    ...
    UnicodeString str2("string2");
    UnicodeString str3("string3");
    UnicodeString str4("string4");
    UnicodeString str5("string5");
...
void testOOListFunctions() {
    ...
    UnicodeString us("12345678901234");
...
void testNullFSs() {
    ...
    uima::lowlevel::TyFSFeature lolBeginPosFeature = lolTypeSys.getFeatureByBaseName(lolAnnotType,
                                                                                     CAS::FEATURE_BASE_NAME_BEGIN);
    uima::lowlevel::TyFSFeature lolLemmaListFeature = lolTypeSys.getFeatureByBaseName(lolAnnotType,
                                                                                      TT::FEATURE_BASE_NAME_LEMMA_ENTRIES);
...
void testFullyQualifiedFeatureNames() {
    ...
    UnicodeString annotBeginPos(CAS::TYPE_NAME_ANNOTATION);
    annotBeginPos.append(":");
    annotBeginPos.append(CAS::FEATURE_BASE_NAME_BEGIN);

    UnicodeString tokenBeginPos(TT::TYPE_NAME_TOKEN_ANNOTATION);
...
void testCASReset() {
    ...
    UnicodeString const& indexID = tcas.getAnnotationIndexID();
...
//   uima::Feature morphIDFeature = lemmaType.getFeatureByBaseName( TT::FEATURE_BASE_NAME_MORPH_ID);
    ...
    UnicodeString str1("str1");
    uima::UnicodeStringRef ref1(str1);

    UnicodeString str2("str2");
