...
//#define UIMA_NO_HASH_CONTAINERS_SUPPORTED 1
...
// input data types
enum dataFormats {
    textFormat, xcasFormat, xmiFormat
};
...
void processInputFiles(AnalysisEngine* pEngine) {
    ...
    for (int i = 0; i < numruns; i++) {
        stringstream str;
        cout << endl << "ThreadId: " << apr_os_thread_current() << " runAECpp::processing start iteration: " << i <<
        endl;
        //uima::ResourceManager::getInstance().getLogger().logMessage(str.str() );
        /* process input */
        util::DirectoryWalk dirwalker(in.c_str());
        if (dirwalker.isValid()) {
            cout << "ThreadId: " << apr_os_thread_current() << " runAECpp::processing all files in directory: " <<
            in.c_str() << endl;
            ...
            if (!randomize) {
                while (dirwalker.isValid()) {
                    ...
                    if (dirwalker.isFile()) {
                        ...
                        if (count % 100 == 0 && count > 0) {
                            str << apr_time_now() << " ThreadId: " << apr_os_thread_current() << " numProcessed=" <<
                            count;
                            cerr << str.str() << endl;
                            uima::ResourceManager::getInstance().getLogger().logMessage(str.str());
                        }
                        ...
                        if (rdelay > 0) {
                            ...
                            cout << "ThreadId: " << apr_os_thread_current() << " runAECpp::processing sleep for " <<
                            howlong << " millis " << endl;
                ...
                for (int i = 0; i < num; i++) {
                    ...
                    stringstream str;
                    if (count % 100 == 0 && count > 0) {
                        str << apr_time_now() << " ThreadId: " << apr_os_thread_current() <<
                        " runAECpp::processing numProcessed=" << count;
                        cerr << str.str() << endl;
                        uima::ResourceManager::getInstance().getLogger().logMessage(str.str());
                    }
