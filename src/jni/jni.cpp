...
//      assert( featureOffsets != NULL );
    ...
    vector<UnicodeString> vecTypeNames, vecFeatureNames, vecIndexNames, vecSubStringValues;
...
JNIEXPORT void JNICALL JAVA_PREFIX(fillCASJNI) (JNIEnv* jeEnv,
  ...
  try {
    ...
    vector<UnicodeString> vecStringTable;
...
//      rs.print(cerr);
    ...
    if (isTCas) {
      ...
      UnicodeString usofa = UnicodeString( ref.getBuffer(), ref.length() );
