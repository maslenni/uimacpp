...
void JNIUtils::throwNewInternalException(JNIEnv* jeEnv, uima::Exception & rException) {
  ...
  UnicodeString usExc(sExc.c_str(), sExc.length());
...
void JNIUtils::createStringVector(JNIEnv* jeEnv, jobjectArray joArray, vector<uima::UnicodeStringRef> & rResult, vector<UnicodeString> & rStringPool) {
