...
namespace uima {
  ...
  * <br>
  ...
  public:
    typedef std::map < UnicodeString, AnnotatorContext * > TyMapDelegateAnCs;
    typedef std::map < UnicodeString, SofaID * > TyMapSofaMappings;
    ...
    **/
     ...
     */
    TyErrorId extractValue(const UnicodeString & paramName, std::string & value) const;
    /** returns string parameter values as UTF-8 string
     * Caller assumes ownership of objects in the vector
     */
    TyErrorId extractValue(const UnicodeString & paramName, std::vector<std::string*> & returnValues) const;

    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, std::string & value) const;
    /** returns string parameter values as UTF-8 string
     * Caller assumes ownership of objects in the vector
     */
    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, std::vector<std::string*> & returnValues) const;
    /** @} */
     ...
     * object.
     **/
    const SofaID & mapToSofaID(const UnicodeString & componentSofaName)  {
      ...
      if (ite != iv_mapSofaMappings.end()) {
        ...
        UnicodeString astr = pSofaid->getComponentSofaName();
    ...
    **/
    /*@{*/
    TyErrorId extractValue(const UnicodeString & paramName, bool & value) const;
    TyErrorId extractValue(const UnicodeString & paramName, std::vector<bool> & returnValues) const;
    TyErrorId extractValue(const UnicodeString & paramName, int & value) const;
    TyErrorId extractValue(const UnicodeString & paramName, size_t & value) const;
    TyErrorId extractValue(const UnicodeString & paramName, std::vector<int> & returnValues) const;
    TyErrorId extractValue(const UnicodeString & paramName, float & value) const;
    TyErrorId extractValue(const UnicodeString & paramName, std::vector<float> & returnValues) const;
    TyErrorId extractValue(const UnicodeString & paramName, UnicodeString & value) const;
    TyErrorId extractValue(const UnicodeString & paramName, std::vector<UnicodeString> & returnValues) const;

    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, bool & value) const;
    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, std::vector<bool> & returnValues) const;
    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, int & value) const;
    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, size_t & value) const;
    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, std::vector<int> & returnValues) const;
    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, float & value) const;
    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, std::vector<float> & returnValues) const;
    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, UnicodeString & value) const;
    TyErrorId extractValue(const UnicodeString & groupName, const UnicodeString & paramName, std::vector<UnicodeString> & returnValues) const;
    /** @} */
     ...
     * TAE specifier
     */
    bool isParameterDefined(const UnicodeString & paramName) const;

    /**
     * Returns true iff the parameter paramName is defined in the
     * group groupName for this TAE specifier
     */
    bool isParameterDefined(const UnicodeString & groupName,
                            const UnicodeString & paramName) const;
    ...
    * object.
    **/
    AnnotatorContext * getDelegate(const UnicodeString & key) const;
    ...
    * AnnotatorContexts
    **/
    AnnotatorContext * extractDelegate(const UnicodeString & key);
    ...
    **/
    const std::set <UnicodeString> getGroupNamesForParameter(const UnicodeString & paramName) const;
    ...
    **/
    ...
    TyErrorId assignValue(const UnicodeString & paramName, const bool & value);
    TyErrorId assignValue(const UnicodeString & paramName, const std::vector< bool > & value);

    TyErrorId assignValue(const UnicodeString & paramName, const size_t & value);
    TyErrorId assignValue(const UnicodeString & paramName, const std::vector< size_t > & value);

    TyErrorId assignValue(const UnicodeString & paramName, const int & value);
    TyErrorId assignValue(const UnicodeString & paramName, const std::vector< int > & value);

    TyErrorId assignValue(const UnicodeString & paramName, const float & value);
    TyErrorId assignValue(const UnicodeString & paramName, const std::vector< float > & value);

    TyErrorId assignValue(const UnicodeString & paramName, const double & value);
    TyErrorId assignValue(const UnicodeString & paramName, const std::vector< double > & value);

    TyErrorId assignValue(const UnicodeString & paramName, const UnicodeString & value);
    TyErrorId assignValue(const UnicodeString & paramName, const std::vector< UnicodeString > & value);

    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const bool & value);
    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const std::vector< bool > & value);

    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const int & value);
    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const std::vector< int > & value);

    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const size_t & value);
    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const std::vector< size_t > & value);

    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const float & value);
    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const std::vector< float > & value);

    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const double & value);
    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const std::vector< double > & value);


    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const UnicodeString & value);
    TyErrorId assignValue(const UnicodeString & groupName, const UnicodeString & paramName, const std::vector< UnicodeString > & value);
  ...
  private:
    ...
    NameValuePair const * findNameValuePair(
      ...
      const UnicodeString & ancKey="") const;

    NameValuePair const * findNameValuePair(const UnicodeString & paramName,
                                            const UnicodeString & ancKey="") const;
...
namespace uima {
   ...
   */
  inline
  TyErrorId AnnotatorContext::extractValue(const UnicodeString & paramName,
  ...
  inline
  TyErrorId AnnotatorContext::extractValue(const UnicodeString & paramName,
      UnicodeString & returnValue) const {
    NameValuePair const * pResult = findNameValuePair(paramName);
    if (pResult != NULL) {// a value has been found
  ...
  inline
  TyErrorId AnnotatorContext::extractValue(const UnicodeString & paramName,
					   std::vector<UnicodeString> & returnValues) const {
    NameValuePair const * pResult = findNameValuePair(paramName);
    if (pResult != NULL) {// a value has been found
  ...
  inline
  TyErrorId AnnotatorContext::extractValue(const UnicodeString & groupName,
      const UnicodeString & paramName,
      bool & returnValue) const {
