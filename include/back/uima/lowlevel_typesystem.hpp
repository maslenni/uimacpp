...
namespace uima {
  namespace lowlevel {
    ...
    protected:
      ...
      std::vector<UnicodeString> iv_vecTypeNames;
      std::vector<UnicodeString> iv_vecTypeCreatorIDs;
      std::vector<UnicodeString> iv_vecFeatureBaseNames;
      std::vector<UnicodeString> iv_vecFeatureCreatorIDs;

      // string sub types
      std::vector<std::vector<UnicodeString> > iv_enumStrings;
      ...
      TyFSType createTypeNoChecks(TyFSType tyParent, UnicodeString const & crName, UnicodeString const & crusCreatorID);
    ...
    public:
      ...
      */
       ...
       */
      TyFSType createType(TyFSType tyParent, UnicodeString const & crName, UnicodeString const & crusCreatorID);

      TyFSType createStringSubtype(UnicodeString const & crName, std::vector<UnicodeString> const & crStrings, UnicodeString const & crusCreatorID);

      /**
       * create a feature on <code>tyIntro</code> with value type <code>tyValueType</code>
       * and name <code>crName</code>.
       */
      TyFSFeature createFeature(TyFSType tyIntro, TyFSType tyValueType, bool multiRef, UnicodeString const & crName, UnicodeString const & crusCreatorID);

      /**
       * create a type from a type info struct.
       */
      TyFSType createType( StTypeInfo const & , UnicodeString const & crusCreatorID);

      /**
       * create a feature from a feature info struct.
       */
      TyFSType createFeature( StFeatureInfo const & , UnicodeString const & crusCreatorID);
      ...
      std::vector<UnicodeString> const & getStringsForStringSubtype(TyFSType type) const;
       ...
       */
      TyFSType getTypeByName(UnicodeString const & crusTypeName) const;

      /**
       * get the name of the type.
       */
      UnicodeString const & getTypeName(TyFSType tyType) const {
        assert( isValidType(tyType) );
       ...
       */
      UnicodeString const & getTypeCreatorID(TyFSType tyType) const {
        assert( isValidType(tyType) );
       ...
       */
      TyFSFeature getFeatureByFullName(UnicodeString const & crusFeatureName) const;

      /**
       * get the feature by its base name w.r.t. to a type it is defined for.
       */
      TyFSFeature getFeatureByBaseName(TyFSType tyType, UnicodeString const & crusFeatureName) const;

      TyFSFeature getFeatureByBaseName(UnicodeString const & crusTypeName, UnicodeString const & crusFeatureName) const;

      /**
       * get the name of the feature.
       */
      UnicodeString const & getFeatureBaseName(TyFSFeature tyFeature) const {
       ...
       */
      UnicodeString getFeatureName(TyFSFeature tyFeature) const;

      /**
       * get the creator id of the feature.
       */
      UnicodeString const & getFeatureCreatorID(TyFSFeature tyFeature) const {
        assert( isValidFeature(tyFeature) );
      ...
      static UnicodeString const ustrCREATOR_ID_SYSTEM;
    };
