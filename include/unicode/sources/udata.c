...
static UDataMemory* udata_cacheDataItem(const char* path, UDataMemory* item, UErrorCode* pErr) {
    ...
    */
    htable = udata_getHashTable();
    umtx_lock(NULL);
    oldValue = (UDataMemory*) uhash_get(htable, path);
    if (oldValue != NULL) {
...
static void udata_pathiter_init(UDataPathIterator* iter, const char* path, const char* pkg,
    ...
    if (pkg == NULL) {
        ...
        if (uprv_strlen(pkg) + 2 > U_DATA_PATHITER_BUFSIZ) {
            iter->packageStub = (char*) uprv_malloc(uprv_strlen(pkg) + 2);
        } else {
            iter->packageStub = iter->packageStubBuf;
        }
    ...
    {
        int32_t maxPathLen = (int32_t) (uprv_strlen(iter->path) + uprv_strlen(item) + uprv_strlen(iter->suffix) +
                                        iter->packageStubLen + 3);
...
//        @@
//        tData.pHeader = (const DataHeader*) &U_ICUDATA_ENTRY_POINT;
//        udata_checkCommonData(&tData, pErrorCode);
        udata_mapIcudt(&tData, "data/icudt36l.dat", "icudt36l");
...
udata_setAppData(const char* path, const void* data, UErrorCode* err) {
    ...
    udm.pHeader = (const DataHeader*) data;
 ...
 *                    an contentEntry in a common file or a separately loaded file,  *
...
doOpenChoice(const char* path, const char* type, const char* name,
    ...
    TinyString tocEntryName; /* contentEntry name in tree format. ex:  'icudt28b/coll/ar.res' */
    TinyString tocEntryPath; /* contentEntry name in path format. ex:  'icudt28b\\coll\\ar.res' */
    ...
    if (path == NULL ||
        ...
        !uprv_strncmp(path, U_ICUDATA_NAME
        U_TREE_SEPARATOR_STRING, /* "icudt26e-" */
                      uprv_strlen(U_ICUDATA_NAME
                      U_TREE_SEPARATOR_STRING)) ||
        !uprv_strncmp(path, U_ICUDATA_ALIAS
        U_TREE_SEPARATOR_STRING, /* "ICUDATA-" */
                      uprv_strlen(U_ICUDATA_ALIAS
                      U_TREE_SEPARATOR_STRING))) {
        isICUData = TRUE;
    }
    ...
    /* setting up the contentEntry name and file name
    ...
    if (U_SUCCESS(*pErrorCode)) {
        if (U_SUCCESS(subErrorCode)) {
            ...
            *pErrorCode = U_FILE_ACCESS_ERROR;
            ...
            /* contentEntry point not found or rejected */
