 ...
 * byte value.
 * Each contentEntry is 32 bits wide, with two formats distinguished by
 * the sign bit (bit 31):
 *
 ...
 * surrogates.
 * All code points for a final state contentEntry take up the same number of code
 ...
 * the contentEntry itself like with roundtrip mappings.
 * For other final entries, they are stored in the code units table if
 * the contentEntry is for a pair of code units.
 ...
 *
 * The interpretation of the bits in each contentEntry is as follows:
 *
 * Bit 31 not set, not a terminal contentEntry ("transitional"):
 * 30..24 next state
 * 23..0  offset delta, to be added up
 *
 * Bit 31 set, terminal ("final") contentEntry:
 ...
 * In all cases, the contentEntry in stage 1 is directly the index of the block of
...
_EBCDICSwapLFNL(UConverterSharedData *sharedData, UErrorCode *pErrorCode) {
    ...
    if(mbcsTable->fromUBytesLength>0) {
         ...
         * trie and find the highest contentEntry, but it was removed in ICU 3.2
    ...
    if(mbcsTable->outputType==MBCS_OUTPUT_EXT_ONLY) {
        ...
        if( sharedData->staticData->conversionType==UCNV_DBCS ||
            ...
            if(baseSharedData->mbcs.outputType==MBCS_OUTPUT_2_SISO) {
                ...
                /* get the dbcs state from the state table contentEntry for SO=0x0e */
...
ucnv_MBCSSingleToUnicodeWithOffsets(UConverterToUnicodeArgs *pArgs,
    ...
    while(source<sourceLimit) {
        ...
        /* MBCS_ENTRY_IS_FINAL(contentEntry) */
...
ucnv_MBCSToUnicodeWithOffsets(UConverterToUnicodeArgs *pArgs,
    ...
    while(source<sourceLimit) {
        ...
        /* set the next state early so that we can reuse the contentEntry variable */
        state=(uint8_t)MBCS_ENTRY_FINAL_STATE(entry); /* typically 0 */
...
ucnv_MBCSSingleSimpleGetNextUChar(UConverterSharedData *sharedData,
    ...
    int32_t contentEntry;
    uint8_t action;

    contentEntry=sharedData->mbcs.stateTable[0][b];
    /* MBCS_ENTRY_IS_FINAL(contentEntry) */

    if(MBCS_ENTRY_FINAL_IS_VALID_DIRECT_16(contentEntry)) {
        /* output BMP code point */
        return (UChar)MBCS_ENTRY_FINAL_VALUE_16(contentEntry);
    }
     ...
     * the most common cases compared to a switch.
     */
    action=(uint8_t)(MBCS_ENTRY_FINAL_ACTION(contentEntry));
    if(action==MBCS_STATE_VALID_DIRECT_20) {
        /* output supplementary code point */
        return 0x10000+MBCS_ENTRY_FINAL_VALUE(contentEntry);
    } else if(action==MBCS_STATE_FALLBACK_DIRECT_16) {
...
getTrail:
            ...
            if(!(MBCS_FROM_U_IS_ROUNDTRIP(stage2Entry, c) ||
                ...
                /*
                 * We allow a 0 byte output if the "assigned" bit is set for this contentEntry.
...
getTrail:
             ...
             * Bits 15..0 of the stage 2 contentEntry contain the stage 3 index
