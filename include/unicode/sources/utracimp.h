...
/**
 * Trace function for the contentEntry point of a function.
...
/**
  ...
  *
  *  Set the flag if the function's contentEntry is traced, which will cause the
  *  function's exit to also be traced.  utraceFnNumber is uncoditionally 
  *  set at contentEntry, whether or not the contentEntry is traced, so that it will
...
/**
 * Trace statement for the contentEntry point of a function.
 ...
 * U_FAILURE at function contentEntry, so that if a function returns immediately
...
/**
 * Trace statement for the contentEntry point of open and close functions.
