...
findConverter(const char* alias, UBool* containsOption, UErrorCode* pErrorCode) {
    ...
    for (; ;) {
        ...
        if (result < 0) {
            ...
            /* Since the gencnval tool folds duplicates into one contentEntry,
...
ucnv_openStandardNames(const char* convName,
    ...
    if (haveAliasData(pErrorCode) && isAlias(convName, pErrorCode)) {
        ...
        if (listOffset < gMainTable.taggedAliasListsSize) {
            ...
            myContext = (UAliasContext*) uprv_malloc(sizeof(UAliasContext));
            if (myContext == NULL) {
                *pErrorCode = U_MEMORY_ALLOCATION_ERROR;
                uprv_free(myEnum);
                return NULL;
...
ucnv_openAllNames(UErrorCode* pErrorCode) {
    ...
    if (haveAliasData(pErrorCode)) {
        ...
        myContext = (uint16_t*) uprv_malloc(sizeof(uint16_t));
        if (myContext == NULL) {
            *pErrorCode = U_MEMORY_ALLOCATION_ERROR;
            uprv_free(myEnum);
            return NULL;
...
io_compareRows(const void* context, const void* left, const void* right) {
    ...
    return (int32_t) uprv_strcmp(
            tempTable->stripForCompare(strippedLeft, chars + 2 * ((const TempRow*) left)->strIndex),
...
ucnv_swapAliases(const UDataSwapper* ds,
    ...
    offsets[converterListIndex] = 2 * (1 + tocLength); /* count two 16-bit units per toc contentEntry */
    ...
    if (length >= 0) {
        ...
        ds->swapInvChars(ds, inTable + offsets[stringTableIndex],
                         2 * (int32_t) (toc[stringTableIndex] + toc[normalizedStringTableIndex]),
