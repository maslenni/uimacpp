 ...
 *----------------------------------------------------------------------------*/
    ...
    uprv_mapFile(UDataMemory *pData, const char *path) {
...
#ifndef U_HPUX
        data=mmap(0, fileLength, PROT_READ, MAP_SHARED,  fd, 0);
#else
        data=mmap(0, fileLength, PROT_READ, MAP_PRIVATE, fd, 0);
#endif
        ...
        pData->map = (char *)data + fileLength;
