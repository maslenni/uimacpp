...
utrie_compact(UNewTrie *trie, UBool overlap, UErrorCode *pErrorCode) {
    ...
    for(start=newStart; start<trie->dataLength;) {
        /*
         * start: index of first contentEntry of current block
 ...
 * Just store the offset (16 bits) if there is any non-initial-value contentEntry.
 *
...
utrie_enum(const UTrie *trie,
    ...
    /* get the enumeration value that corresponds to an initial-value trie data contentEntry */
